...
- 当前 session 对话脉络是用户连续体验。它可以自动注入，但必须短、结构化，只总结同一个 session 内最近的可见对话。
- 当前任务工作记忆是 Agent 执行状态。它可以自动注入，但必须短、结构化，并且只绑定当前目标。
- 原始历史是证据库。它必须持久、可查询，但不能整段自动注入当前目标。
- 当前上下文是模型当下工作的桌面。它应该包含当前用户输入、同 session 对话脉络、当前任务工作记忆和最小可用工作集，而不是旧任务残余。

不要重建那种把旧任务混进当下的压缩对话记忆。也不要让模型每一轮都像空桌面一样失忆。正确边界是：同 session 对话脉络给用户连续感；当前任务工作记忆给 Agent 执行连续性；完整历史留在取证工具里；是否查询历史由 Lead 判断。
