...
function buildProjectEnvTemplate(example: boolean): string {
  ...
  return [
    ...
    "",
    "# 当前默认示例：OpenAI 中转 + GPT-5.4",
    "# 先把 provider/base URL/model 写在当前项目自己的 `.athlete/.env`，不要再依赖外部认证目录。",
    "# 如果 doctor 发现这个中转入口实际需要 `/v1`，把最终探测成功的 URL 原样写回 ATHLETE_BASE_URL。",
    "ATHLETE_PROVIDER=openai",
    athleteApiKeyLine,
    "ATHLETE_BASE_URL=https://w.ciykj.cn",
    "ATHLETE_MODEL=gpt-5.4",
    "",
    "# 本仓库的 Playwright MCP 默认配置",
    "ATHLETE_MCP_ENABLED=1",
    "ATHLETE_MCP_PLAYWRIGHT_ENABLED=1",
    "# 默认使用 Playwright 自带的 Chromium；只有你明确想锁定某个 channel 时才改成 chrome / msedge / firefox / webkit。",
    ...
    "# ATHLETE_PROVIDER=openai-compatible",
    "# ATHLETE_API_KEY=replace-with-your-key",
