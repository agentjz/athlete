...
export async function loginWeixin(options: {
  ...
  const credentials = await client.loginWithQr({
    ...
    onScanned() {
      console.log("[weixin] 二维码已扫码，请在微信里确认登录。");
    },
    onExpired(attempt, maxAttempts) {
      console.log(`[weixin] 二维码已过期，正在重试 ${attempt}/${maxAttempts}。`);
    },
  });
  ...
  console.log(
    `[weixin] 登录完成 user=${credentials.userId ?? "unknown"} bot=${credentials.botId ?? "unknown"} state=${runtime.stateDir}`,
  );
}
...
export async function logoutWeixin(options: {
  ...
  console.log(`[weixin] 已退出登录 state=${runtime.stateDir}`);
}
...
export function registerWeixinCommands(
  ...
  weixinCommand
    ...
    .action(async () => {
      ...
      console.log(
        `[weixin] 正在启动私聊服务 allowed=${runtime.config.weixin.allowedUserIds.join(",")} state=${runtime.config.weixin.stateDir}`,
      );
