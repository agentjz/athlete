...
export interface WeixinDeliveryObserver {
...
export class WeixinDeliveryQueue {
  ...
  private readonly observers = new Set<WeixinDeliveryObserver>();
    ...
    await this.withLock(async () => {
      ...
      for (const entry of entries) {
        ...
        if (!token) {
          ...
          this.notifyBlocked(entry, blockedReason);
          continue;
        }
        ...
        try {
          ...
          if (isContextTokenDeliveryFailure(error)) {
            ...
            this.notifyBlocked(entry, "context_token_invalid");
            continue;
          }
  ...
  subscribe(observer: WeixinDeliveryObserver): () => void {
  ...
  private notifyAccepted(entry: WeixinDeliveryEntry): void {
    for (const observer of this.observers) {
      observer.onAccepted?.(entry);
    }
  }

  private notifyDeliveryFailed(entry: WeixinDeliveryEntry, error: unknown): void {
    for (const observer of this.observers) {
      observer.onDeliveryFailed?.(entry, error);
    }
  }

  private notifyBlocked(entry: WeixinDeliveryEntry, reason: WeixinDeliveryBlockedReason): void {
    for (const observer of this.observers) {
      observer.onBlocked?.(entry, reason);
    }
  }
}
