...
import type { TelegramIgnoredUpdate, TelegramPrivateMessage, TelegramUpdate } from "./types.js";
...
export class TelegramService {
  ...
  constructor(private readonly options: TelegramServiceOptions) {
    ...
    this.observabilityRootDir = resolveHostStateRoot(options.config.telegram.stateDir, options.cwd);
...
function describeIgnoredTelegramUpdate(update: TelegramIgnoredUpdate): string {
  ...
  switch (update.reason) {
    ...
    case "unauthorized_user":
      fragments.push("Sender is not in ATHLETE_TELEGRAM_ALLOWED_USER_IDS.");
      break;
    case "empty_message":
