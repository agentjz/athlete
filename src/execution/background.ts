...
export class BackgroundJobStore {
  ...
  async complete(
    ...
    const closedStatus = input.status === "completed"
      ? "completed"
...
export async function terminateBackgroundJob(input: {
  rootDir: string;
  jobId: string;
  terminatedBy: string;
  ...
  const job = await store.load(input.jobId);
  if (job.status === "completed" || job.status === "failed" || job.status === "timed_out" || job.status === "aborted") {
    return {
      job,
  ...
  if (typeof job.pid === "number") {
    terminateProcess(job.pid, input.signal === "kill" ? "SIGKILL" : "SIGTERM");
  }
  ...
  const closed = await closeExecution({
    ...
    executionId: job.id,
    status: "aborted",
    summary: `background execution aborted by ${input.terminatedBy}`,
    output: `Background execution '${job.id}' was terminated by ${input.terminatedBy}.`,
    statusDetail: input.signal === "kill" ? "terminated_forcefully" : "terminated_by_request",
  });

  return {
    job: mapExecutionToBackgroundJob(assertBackgroundExecution(closed)),
