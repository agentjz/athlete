...
import { launchSubagentWorkerExecution } from "../capabilities/subagent/launch.js";
  ...
  switch (input.decision.action) {
    case "delegate_subagent": {
      ...
      const { execution, pid } = await launchSubagentWorkerExecution({
        ...
        objective: buildSubagentPrompt(input.analysis, task.record.id, task.record.subject),
        scope: `Task #${task.record.id}: ${task.record.subject}`,
        expectedOutput: "Return a CloseoutContract with concrete evidence for Lead review.",
