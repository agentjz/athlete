...
import {
  createSubagentBudgetExceededReason,
  createSubagentBudgetTracker,
  resolveSubagentBudget,
  type SubagentBudgetExceededReason,
  type SubagentBudgetTracker,
} from "./budget.js";
import { isSubagentBudgetExceededError, SubagentBudgetExceededError } from "./errors.js";
...
export interface RunSubagentTaskResult {
  ...
  budgetExceededReason?: SubagentBudgetExceededReason;
}
...
export async function runSubagentTask(
  ...
  const budget = resolveSubagentBudget(options.config.delegationMode);
  const budgetTracker = createSubagentBudgetTracker(budget);
  ...
  try {
    const result = await runAgentTurn({
      ...
      sessionStore,
      toolRegistry,
      callbacks: createBudgetGuardedCallbacks(reporter.createCallbacks(), budgetTracker),
    ...
    const budgetReason = resolveBudgetExceededReason(error, budgetTracker, elapsedBudgetExceeded);
    if (budgetReason) {
      reporter.fail(budgetReason.message);
      await closeExecution({
        rootDir: projectContext.stateRootDir,
        executionId: execution.id,
        status: "paused",
        summary: "subagent execution paused: budget exhausted",
        output: JSON.stringify(budgetReason, null, 2),
        pauseReason: budgetReason.message,
        statusDetail: budgetReason.code,
        notifyRequester: false,
      }).catch(() => null);

      return {
        executionId: execution.id,
        content: budgetReason.message,
        status: "budget_exhausted",
        budgetExceededReason: budgetReason,
      };
    }
...
function createBudgetGuardedCallbacks(
  callbacks: AgentCallbacks,
  budgetTracker: SubagentBudgetTracker,
): AgentCallbacks {
  const guard = (reason: SubagentBudgetExceededReason | undefined): void => {
    if (reason) {
  ...
  return {
    ...callbacks,
    onStatus: (text) => {
      guard(budgetTracker.evaluate());
      callbacks.onStatus?.(text);
    },
    onModelWaitStart: () => {
      guard(budgetTracker.noteModelTurn());
    ...
    onToolCall: (name, args) => {
      guard(budgetTracker.noteToolCall(name));
...
function resolveBudgetExceededReason(
  error: unknown,
  budgetTracker: SubagentBudgetTracker,
  elapsedBudgetExceeded: boolean,
): SubagentBudgetExceededReason | undefined {
  if (isSubagentBudgetExceededError(error)) {
    return error.reason;
  }

  if (!elapsedBudgetExceeded) {
    return undefined;
  }

  const snapshot = budgetTracker.snapshot();
  snapshot.elapsedMs = Math.max(snapshot.elapsedMs, snapshot.maxElapsedMs + 1);
  return createSubagentBudgetExceededReason("elapsed_ms", snapshot);
}
