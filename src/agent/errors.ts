...
export function getErrorMessage(error: unknown): string {
  ...
  if (
    ...
    lower.includes("set athlete_api_key")
  ) {
    return "用户可修复错误：未找到 API key。请在当前项目的 `.athlete/.env` 里设置 `ATHLETE_API_KEY`。";
  }
  ...
  if (
    ...
    return "环境错误：网络连接失败，当前 provider/base URL 不可达。请检查网络、代理设置，或确认 `ATHLETE_BASE_URL` 可访问。";
  }

  if (status === 404 || lower.includes("returned 404")) {
    return "用户可修复错误：provider 入口返回 404。请检查 `ATHLETE_BASE_URL` 是否是正确的 OpenAI-compatible API 根地址。";
  }
