...
> {
  ...
  if (!hasVisibleAssistantResult(params.response.content)) {
    ...
    const session = await persistCheckpointTransition(
      await params.options.sessionStore.appendMessages(params.session, [
        ...
        createMessage(
          "user",
          createInternalReminder(
            "You returned no user-visible task result. Do not finalize yet. Either continue the work or provide a concrete final status with evidence.",
          ),
        ),
      ]),
      params.options.sessionStore,
    ...
    params.options.callbacks?.onStatus?.("Assistant returned no visible result. Asking the model to continue or report concretely...");
    return {
      kind: "continue",
      session,
      validationReminderInjected: params.validationReminderInjected,
