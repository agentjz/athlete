...
import { buildDiffPreview, normalizeDiffPath, okResult, parseArgs, readString } from "../../core/shared.js";
...
export const applyPatchTool: RegisteredTool = {
  ...
  async execute(rawArgs, context) {
    ...
    const diagnostics = await collectWriteDiagnostics(changedPaths);
    const diffPreview = truncateText(
      operations.map((operation) => operation.preview).filter(Boolean).join("\n"),
      8_000,
    );
