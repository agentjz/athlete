...
- 运行节奏：Lead 可以用一个工具批次查委派状态；如果后台或队友还没回来，机器会要求 Lead 先做一项不依赖该结果的非冲突工作，再允许下一轮查委派状态。
- 等待期间的理想体感不是“总指挥停机”，也不是“总指挥只会反复查状态”，而是 `查一批委派状态 -> Lead 自己推进合流准备/证据整理/非冲突检查 -> 再查一批委派状态 -> 合流复核`。当前目标的 task / worktree 收口查询不算委派状态轮询。
...
- 委派进行中时，Lead 不对外停机；主循环必须在查状态和 Lead 自己推进之间交替，状态变化后再合流复核。
