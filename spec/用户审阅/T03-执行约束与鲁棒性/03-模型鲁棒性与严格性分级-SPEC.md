...
- 关键词不再单独触发委派。
- 委派必须先通过“必要性评估”，再通过机器策略闸门。
- 每次委派完成或超限后，必须回主控复核，再决定下一步。

### 3.5 委派等待语义（增加）

- 等待信号仍使用结构化原因：`pause.orchestrator_waiting_for_delegated_work`（用于可观测与审计）。
- Lead 主路径：命中 `wait_for_existing_work` 时不把 turn 终止为对外 `paused`，而是在主循环内持续轮询控制面状态，直到出现可推进状态再继续路由/执行。
- 轮询期间必须持续输出可读状态，让用户体感为“总指挥仍在运行”，而非“看起来停机”。
...
- 委派不再由关键词单点触发，必须经过双闸门。
- subagent 任一预算超限都会强制返回 Lead 且可结构化审计。
- 委派后必须先回主控复核，不能同链路无限深挖。
- 委派进行中时，Lead 不再以 `pause.orchestrator_waiting_for_delegated_work` 对外停机；主循环会持续轮询并在状态变化后继续推进。
