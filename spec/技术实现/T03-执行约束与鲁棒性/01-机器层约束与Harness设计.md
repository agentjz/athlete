...
`delegatedWaitRhythm.ts` 专门处理委派等待节奏：active delegated work 仍未完成时，Lead 可以用一个工具批次查委派状态；如果结果仍未回来，下一轮必须先做合流准备、已有证据整理或非冲突检查，再允许下一次委派状态查询。task / worktree 这类当前目标收口状态查询不算委派状态轮询，不能把 Lead 逼去翻 `.deadmouse` 内部文件。
...
- active delegated work 不再让 lead loop 内部空等，改为 Lead 合流准备输入，并由 delegated wait rhythm 阻断连续状态查询空转
...
- Lead loop 遇到活跃委派工作时推动合流准备，不做无限等待；委派结果未回来时，Lead 在查状态和非冲突自身推进之间交替。
...
- 循环守卫按结果签名判断无进展重复：静态读取同结果才阻断，结果变化视为进展；活状态轮询不因同参重复被通用 loop guard 硬拦，但委派等待场景会额外要求 Lead 在两批委派状态查询之间先做自身推进。
