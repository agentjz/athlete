...
🧠 在智能体设计里，如果旧任务被整段塞进当下，旧惯性就会伪装成当前意图；如果同一个 session 的对话脉络完全消失，用户体验又会像每句话都重新开局。小猫智能体把它们拆开：同 session 的近期对话脉络提供连续感，当前任务工作记忆负责执行续航，完整历史、最终输出、工具产物、运行事件、变更记录、trace 和 ledger 留在证据库里，按需查询。生态只是可用能力，不是行动意图；只有领导者显式选择，能力才会进入执行。
...
<p align="center">
  <img src="assets/community-group.jpg" alt="微信交流群" width="300">
</p>
...
| --- | --- | --- |
| `read` | 读取带行号的局部文本窗口。 | ✅ |
| `edit` | 用当前精确文本替换来编辑已有文件。 | ✅ |
| `write` | 新建或完整重写文件。 | ✅ |
| `bash` | 运行搜索、列目录、Git diff/status、构建和测试等 shell 命令。 | ✅ |
