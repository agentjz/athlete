...
optional_tools: mcp_playwright_browser_take_screenshot, download_url, http_probe, write_file, read_file
...
6. If the browser reveals a direct document URL, prefer `download_url` to acquire the file locally before switching into document-reading tools.
7. Use `http_probe` or other real verification steps when the task requires proving that a generated endpoint or page is actually reachable.
