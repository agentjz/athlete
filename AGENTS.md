...
先阅读 `spec/用户审阅/项目规则/用户偏好.md`，把项目所有者偏好转成当前任务的正向行动标准。

然后 research 当前仓库。所有判断、决策、修改和结论都基于当前代码、测试和 spec 事实。

测试写当前架构的正向事实，覆盖真实产品行为。

## 真相源

- `spec/用户审阅/项目规则/用户偏好.md`：项目所有者偏好边界。
...
git commit / git push 只在项目所有者明确要求时执行。
