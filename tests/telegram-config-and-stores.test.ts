...
test("telegram update filter accepts only authorized private chats", () => {
  ...
  assert.equal(accepted.text, "hello athlete");
...
test("telegram message chunking splits oversized replies without losing content", () => {
  const text = [
    "Athlete can now stream results back to Telegram.",
    "",
...
test("telegram console logger prints operator-friendly status lines and trims reply previews", () => {
  ...
  try {
    ...
    logger.info("queued text reply", {
      chatId: 2001,
      detail: `这是一个非常长的回复预览 ${"A".repeat(120)} SHOULD_NOT_APPEAR`,
    });
  } finally {
...
test("resolveRuntimeConfig reads ATHLETE_TELEGRAM_PROXY_URL from the project .env file", async (t) => {
  ...
  await fs.writeFile(
    ...
    [
      ...
      "ATHLETE_TELEGRAM_TOKEN=test-telegram-token",
      "ATHLETE_TELEGRAM_ALLOWED_USER_IDS=1001",
  ...
  const previous = {
    ...
    ATHLETE_TELEGRAM_TOKEN: process.env.ATHLETE_TELEGRAM_TOKEN,
    ATHLETE_TELEGRAM_ALLOWED_USER_IDS: process.env.ATHLETE_TELEGRAM_ALLOWED_USER_IDS,
    ATHLETE_TELEGRAM_PROXY_URL: process.env.ATHLETE_TELEGRAM_PROXY_URL,
  };

  try {
    delete process.env.ATHLETE_API_KEY;
    delete process.env.ATHLETE_TELEGRAM_TOKEN;
    delete process.env.ATHLETE_TELEGRAM_ALLOWED_USER_IDS;
    delete process.env.ATHLETE_TELEGRAM_PROXY_URL;
