...
test("telegram service downloads inbound files, persists attachment metadata, and routes the file into the turn context", async (t) => {
  ...
  const service = new TelegramService({
    ...
    runTurn: async (options) => {
      ...
      options.callbacks?.onAssistantDone?.("已收到文件并开始分析。");
      return {
  ...
  assert.match(seenInputs[0] ?? "", /请分析我刚发的文件/);
...
test.skip("telegram service only streams tool-use, todo previews, and the final reply", { concurrency: false }, async (t) => {
  ...
  const runtime = createTestRuntimeConfig(root);
  const telegram = createTelegramConfig(root);
  const bot = new EnhancedFakeTelegramBotApiClient([[createPrivateTextUpdate(90, "run a detailed task")]]);
  const sessionStore = new SessionStore(runtime.paths.sessionsDir);
  const sessionMapStore = new FileTelegramSessionMapStore(path.join(telegram.stateDir, "session-map.json"));
  const offsetStore = new FileTelegramOffsetStore(path.join(telegram.stateDir, "offset.json"));
  const deliveryQueue = new TelegramDeliveryQueue({
  ...
  const rawToolOutput = "SECRET_TOOL_OUTPUT::" + "X".repeat(5_000);
  const terminalLines: string[] = [];
  const originalLog = console.log;
  const originalError = console.error;
  console.log = (...args: unknown[]) => {
    terminalLines.push(args.map((part) => String(part)).join(" "));
  };
  ...
  try {
    const service = new TelegramService({
      ...
      deliveryQueue,
      runTurn: async (options) => {
        options.callbacks?.onStatus?.("正在分析任务");
        options.callbacks?.onReasoningDelta?.("先理解任务范围，");
        options.callbacks?.onReasoningDelta?.("再决定要读哪些文件。");
        options.callbacks?.onToolCall?.("search_files", "{\"pattern\":\"TODO\"}");
        options.callbacks?.onToolResult?.("search_files", rawToolOutput);
        options.callbacks?.onToolCall?.(
          "todo_write",
          JSON.stringify({
            items: [
              { id: "1", text: "理解需求", status: "completed" },
              { id: "2", text: "整理输出", status: "in_progress" },
            ],
          }),
        );
        ...
        options.callbacks?.onStatus?.("正在生成结果");
    ...
    await service.runOnce();
    await service.waitForIdle();
    await waitFor(() => bot.sentMessages.length > 0 || bot.editedMessages.length > 0);
  } finally {
  ...
  const progressTranscript = [
    ...
    ...bot.editedMessages.map((entry) => entry.text),
  ].join("\n");
  const terminalTranscript = terminalLines.join("\n");

  assert.deepEqual(bot.editedMessages, []);
  assert.deepEqual(
    bot.sentMessages.map((entry) => entry.text),
    [
      "先理解任务范围，再决定要读哪些文件。",
  ...
  assert.doesNotMatch(progressTranscript, /SECRET_TOOL_OUTPUT::/);
  assert.match(terminalTranscript, /received|message/i);
  assert.match(terminalTranscript, /session/i);
  assert.match(terminalTranscript, /search_files/);
  assert.match(terminalTranscript, /complete|success|sent/i);
...
test("telegram service can generate a file and send it back through the telegram tool bridge", async (t) => {
  ...
  const service = new TelegramService({
    ...
    runTurn: async (options) => {
      ...
      options.callbacks?.onAssistantDone?.("已发送 report.md");
      return {
        session: await options.sessionStore.save(options.session),
...
test("telegram service can locate a workspace file and send it back through the telegram tool bridge", async (t) => {
  ...
  const service = new TelegramService({
    ...
    runTurn: async (options) => {
      ...
      options.callbacks?.onAssistantDone?.("文件已发送");
      return {
        session: await options.sessionStore.save(options.session),
        changedPaths: [],
        verificationAttempted: false,
...
test("README documents Telegram bot setup, /stop semantics, file transfer, and terminal logs for first-time users", async () => {
  ...
  assert.match(readme, /athlete telegram serve/i);
  assert.match(readme, /ATHLETE_TELEGRAM_TOKEN/);
  assert.match(readme, /ATHLETE_TELEGRAM_ALLOWED_USER_IDS/);
