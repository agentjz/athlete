...
test("telegram turn display emits assistant stages, todo previews, and the final assistant reply while hiding non-todo tool previews", async () => {
...
test("telegram turn display emits non-streamed assistant stage text before todo previews while hiding non-todo tool previews", async () => {
  const messages: Array<{ chatId: number; text: string }> = [];
  ...
  display.callbacks.onAssistantDone?.("Inspection complete.");
  await display.flush();
  display.dispose();

  assert.deepEqual(messages, [
    { chatId: 99, text: "I will inspect the directory first." },
    { chatId: 99, text: "[x] #1: same todo preview" },
    { chatId: 99, text: "Inspection complete." },
  ]);
});

test("telegram turn display suppresses non-todo tool result previews", async () => {
  const messages: Array<{ chatId: number; text: string }> = [];
