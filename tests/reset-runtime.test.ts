...
test("reset clears project runtime state but preserves env files and unrelated sessions", { concurrency: false }, async (t) => {
  ...
  const projectStateDir = path.join(root, ".athlete");

  await fs.mkdir(projectStateDir, { recursive: true });
  await fs.writeFile(path.join(projectStateDir, ".env"), "ATHLETE_API_KEY=test-key\n", "utf8");
  await fs.writeFile(path.join(projectStateDir, ".env.example"), "ATHLETE_API_KEY=\n", "utf8");
...
test("reset command is available through slash-prefixed local command", async (t) => {
  ...
  await fs.writeFile(path.join(root, ".athlete", ".env"), "ATHLETE_API_KEY=test-key\n", "utf8");
  ...
  assert.equal(result, "quit");
  assert.equal(await pathExists(path.join(root, ".athlete", ".env")), true);
});
