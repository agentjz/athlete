...
test("telegram service runs private inbound messages through the Athlete turn/session runtime and replies through delivery", async (t) => {
  ...
  assert.equal(
    infoLogs.some(
      (entry) =>
        ...
        /ATHLETE_TELEGRAM_ALLOWED_USER_IDS/i.test(entry.detail ?? ""),
    ),
    true,
  );
});
