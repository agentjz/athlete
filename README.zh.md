...
Kitty 不是让机器层替 Lead 模型干活，也不是让机器层变成第二个总指挥。它的设计是给 Lead 配上账本、边界、循环守卫和证据记录：用户给目标，Lead 负责理解目标、选路线、调工具、派 team 或 subagent、开后台、回收结果、判断下一步；机器层负责把这些执行行为变成有记录、有状态、有证据的过程。

方向盘始终在 Lead 手里，但 pending 不能假装完成，执行通道不能无限跑，证据不能伪装成通过，账本不能伪装成策略。简单说，Kitty 不是自动驾驶，也不是审批系统；它是一个把大模型执行过程约束到可持续、可恢复、可追溯状态的 agent harness。
...
| 命令 | 含义 |
| --- | --- |
| `npm install -g @jun133/kitty` | 全局安装 CLI |
| `kitty init` | 初始化当前项目的 `.kitty/` |
| `kitty` | 进入交互模式 |
| `kitty "帮我看看这个项目"` | 执行一次任务 |
| `kitty run "帮我看看这个项目"` | 显式执行一次任务 |
| `kitty resume [sessionId]` | 恢复最近一次或指定会话 |
| `kitty sessions -n 20` | 查看最近会话 |
| `kitty diff [path]` | 查看当前项目 Git diff |
| `kitty changes [changeId]` | 查看变更记录 |
| `kitty undo [changeId]` | 回滚最近一次或指定变更 |
| `kitty config show` | 查看当前配置 |
| `kitty config path` | 查看配置文件路径 |
| `kitty doctor` | 检查本地配置和 API 连通性 |
